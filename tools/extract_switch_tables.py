#!/usr/bin/env python3
"""
Extract switch table data from pe_image.bin and generate ReXGlue TOML config.
...
def main():
    ...
    # are in a different (larger) function. ReXGlue codegen can't generate goto for
    # cross-function jumps, so exclude these and let native indirect calls handle them.
    # Load from JSON file if provided, otherwise empty
    EXCLUDE_BCTRS = set()
    if args.excludes:
    ...
    for sw in switches:
        ...
        info = sw['info']
        print(f"[[switch_tables]]")
        print(f"address = 0x{sw['bctr_addr']:08X}")
        print(f"register = {info['index_reg']}")
